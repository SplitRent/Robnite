...
- **Deterministic grid building** — wall, floor, ramp and cone on a global grid in Fortnite proportions (5.12 m × 3.84 m tiles). Like Fortnite, pieces are placed around the player: walls on the nearest grid line in front of you, floors / ramps / cones in your cell or the next one, with your aim picking the level. The ghost preview *is* the placed piece: the client sends exactly the previewed target and the simulation places it there or not at all. Materials (wood/stone/metal) with build-up health, validation (occupancy, range, materials, players, world, terrain, support), structural collapse, rotation, hold-to-build.
